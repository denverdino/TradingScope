...
def integrate_news_filtering(original_get_stock_news_em):
    ...
    def filtered_get_stock_news_em(
        ...
        try:
            ...
            try:
                # 导入过滤器
                from tradingscope.utils.enhanced_news_filter import create_enhanced_news_filter
...
def patch_akshare_utils():
    ...
    """
    try:
        from tradingscope.dataflows import akshare_utils
        ...
        if not hasattr(akshare_utils, "_original_get_stock_news_em"):
            ...
            # 应用过滤装饰器
            akshare_utils.get_stock_news_em = integrate_news_filtering(akshare_utils._original_get_stock_news_em)
...
def create_filtered_realtime_news_function():
    ...
    def get_filtered_realtime_stock_news(ticker: str, curr_date: str, hours_back: int = 6, enable_filter: bool = True, min_score: float = 30) -> str:
        """
        增强版实时新闻获取函数，集成新闻过滤
            ...
            if any(suffix in ticker for suffix in [".SH", ".SZ", ".SS", ".XSHE", ".XSHG"]) or ("." not in ticker and ticker.isdigit()):
                ...
                try:
                    ...
                    if enable_filter and not original_news_df.empty:
                        # 应用新闻过滤
                        from tradingscope.utils.news_filter import create_news_filter
                    ...
                    if not filtered_news_df.empty:
                        ...
                        # 添加新闻内容
                        for _idx, (_, row) in enumerate(filtered_news_df.iterrows()):
                        ...
                        logger.warning("[增强实时新闻] 过滤后无符合条件的新闻，返回原始报告")
                        return original_report
