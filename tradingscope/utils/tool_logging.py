...
# 导入日志模块
from .logging_manager import get_logger, get_logger_manager
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                tool_logger.error(
                    ...
                    extra={"tool_name": name, "event_type": "tool_call_error", "duration": duration, "error": str(e), "timestamp": datetime.now().isoformat()},
                    exc_info=True,
                )
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            tool_logger.info(
                ...
                extra={"data_source": source_name, "symbol": symbol, "event_type": "data_source_call", "timestamp": datetime.now().isoformat()},
            )
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            tool_logger.info(
                ...
                extra={"llm_provider": provider, "llm_model": model, "event_type": "llm_call_start", "timestamp": datetime.now().isoformat()},
            )
...
def log_tool_usage(tool_name: str, symbol: str = None, **extra_data):
    ...
    Args:
        ...
        **extra_data: 额外的数据
    """
    extra = {"tool_name": tool_name, "event_type": "tool_usage", "timestamp": datetime.now().isoformat(), **extra_data}
...
def log_analysis_step(step_name: str, symbol: str, **extra_data):
    ...
    Args:
        ...
        **extra_data: 额外的数据
    """
    extra = {"step_name": step_name, "symbol": symbol, "event_type": "analysis_step", "timestamp": datetime.now().isoformat(), **extra_data}
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            logger_manager.log_module_start(
                tool_logger, module_name, symbol, actual_session_id, function_name=func.__name__, args_count=len(args), kwargs_keys=list(kwargs.keys())
            )
            ...
            try:
                ...
                logger_manager.log_module_complete(
                    tool_logger, module_name, symbol, actual_session_id, duration, success=True, result_length=result_length, function_name=func.__name__
                )
                ...
                # 记录模块错误
                logger_manager.log_module_error(tool_logger, module_name, symbol, actual_session_id, duration, str(e), function_name=func.__name__)
