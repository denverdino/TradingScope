...
_bootstrap_logger = logging.getLogger("tradingscope.logging_manager")
...
class tradingscopeLogger:
    ...
    def _load_default_config(self) -> Dict[str, Any]:
        ...
        # 从环境变量获取配置
        log_level = os.getenv("tradingscope_LOG_LEVEL", "INFO").upper()
        log_dir = os.getenv("tradingscope_LOG_DIR", "./logs")
        ...
        return {
            ...
            "handlers": {
                ...
                "structured": {"enabled": False, "level": "INFO", "directory": log_dir},  # 默认关闭，可通过环境变量启用
            },
            "loggers": {
                "tradingscope": {"level": log_level},
                "web": {"level": log_level},
            ...
            "docker": {"enabled": os.getenv("DOCKER_CONTAINER", "false").lower() == "true", "stdout_only": True},  # Docker环境只输出到stdout
        }

    def _load_config_file(self) -> Optional[Dict[str, Any]]:
        """从配置文件加载日志配置"""
        # 确定配置文件路径
        config_paths = ["config/logging_docker.toml" if os.getenv("DOCKER_CONTAINER") == "true" else None, "config/logging.toml", "./logging.toml"]
    ...
    def _add_file_handler(self, logger: logging.Logger):
        ...
        log_dir = Path(self.config["handlers"]["file"]["directory"])
        log_file = log_dir / "tradingscope.log"
    ...
    def _add_structured_handler(self, logger: logging.Logger):
        ...
        log_file = log_dir / "tradingscope_structured.log"

        structured_handler = logging.handlers.RotatingFileHandler(log_file, maxBytes=self._parse_size("10MB"), backupCount=3, encoding="utf-8")
    ...
    def log_analysis_complete(self, logger: logging.Logger, stock_symbol: str, analysis_type: str, session_id: str, duration: float, cost: float = 0):
        """记录分析完成"""
        logger.info(
    ...
    def log_module_error(self, logger: logging.Logger, module_name: str, stock_symbol: str, session_id: str, duration: float, error: str, **extra_data):
        """记录模块分析错误"""
        logger.error(
    ...
    def log_token_usage(self, logger: logging.Logger, provider: str, model: str, input_tokens: int, output_tokens: int, cost: float, session_id: str):
        """记录Token使用"""
        logger.info(
...
# 全局日志管理器实例
_logger_manager: Optional[tradingscopeLogger] = None


def get_logger_manager() -> tradingscopeLogger:
    """获取全局日志管理器实例"""
    global _logger_manager
    if _logger_manager is None:
        _logger_manager = tradingscopeLogger()
...
def setup_logging(config: Optional[Dict[str, Any]] = None):
    """设置项目日志系统（便捷函数）"""
    global _logger_manager
    _logger_manager = tradingscopeLogger(config)
    return _logger_manager
