...
class EnhancedNewsFilter(NewsRelevanceFilter):
    ...
    def calculate_enhanced_relevance_score(self, title: str, content: str) -> Dict[str, float]:
        ...
        # 4. 综合评分（加权平均）
        weights = {"rule": 0.4, "semantic": 0.35, "classification": 0.25}  # 规则过滤权重40%  # 语义相似度权重35%  # 分类模型权重25%

        final_score = weights["rule"] * rule_score + weights["semantic"] * scores["semantic_score"] + weights["classification"] * scores["classification_score"]
...
if __name__ == "__main__":
    ...
    test_news = pd.DataFrame(
        [
            {"新闻标题": "招商银行发布2024年第三季度业绩报告", "新闻内容": "招商银行今日发布第三季度财报，净利润同比增长8%，资产质量持续改善..."},
            {"新闻标题": "上证180ETF指数基金（530280）自带杠铃策略", "新闻内容": "数据显示，上证180指数前十大权重股分别为贵州茅台、招商银行600036..."},
            {"新闻标题": "银行ETF指数(512730)多只成分股上涨", "新闻内容": "银行板块今日表现强势，招商银行、工商银行等多只成分股上涨..."},
            {"新闻标题": "招商银行与某科技公司签署战略合作协议", "新闻内容": "招商银行宣布与知名科技公司达成战略合作，将在数字化转型方面深度合作..."},
        ]
    )
