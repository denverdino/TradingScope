...
def init_logging(config_override: Optional[dict] = None) -> None:
    ...
    # 获取初始化日志器
    logger = get_logger("tradingscope.init")

    # 记录初始化信息
    logger.info("🚀 tradingscope-CN 日志系统初始化完成")
...
def log_startup_info():
    ...
    env_info = {
        ...
        "tradingscope_LOG_LEVEL": os.getenv("tradingscope_LOG_LEVEL", "INFO"),
        "tradingscope_LOG_DIR": os.getenv("tradingscope_LOG_DIR", "./logs"),
    }
...
def log_shutdown_info():
    """记录应用关闭信息"""
    logger = get_logger("tradingscope.shutdown")

    logger.info("=" * 60)
    logger.info("🛑 tradingscope-CN 关闭")
    logger.info("=" * 60)
