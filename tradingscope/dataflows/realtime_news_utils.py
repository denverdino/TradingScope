...
class RealtimeNewsAggregator:
    ...
    def __init__(self):
        self.headers = {"User-Agent": "tradingscope-CN/1.0"}
    ...
    def _get_finnhub_realtime_news(self, ticker: str, hours_back: int) -> List[NewsItem]:
        ...
        try:
            ...
            params = {"symbol": ticker, "from": start_time.strftime("%Y-%m-%d"), "to": end_time.strftime("%Y-%m-%d"), "token": self.finnhub_key}
    ...
    def _get_chinese_finance_news(self, ticker: str, hours_back: int) -> List[NewsItem]:
        ...
        try:
            ...
            try:
                logger.info("[中文财经新闻] 尝试导入 AKShare 工具")
    ...
    def _assess_news_urgency(self, title: str, content: str) -> str:
        ...
        # 中等紧急度关键词
        medium_urgency_keywords = ["earnings", "report", "announce", "launch", "merger", "acquisition", "财报", "发布", "宣布", "并购", "收购"]
...
def get_realtime_stock_news(ticker: str, curr_date: str, hours_back: int = 6) -> str:
    ...
    if is_china_stock:
        ...
        try:
            logger.info("[新闻分析] 尝试导入 akshare_utils.get_stock_news_em")
            from .akshare_utils import get_stock_news_em

            logger.info("[新闻分析] 成功导入 get_stock_news_em 函数")
    ...
    # 备用方案2: 尝试使用Google新闻
    try:
        from tradingscope.dataflows.interface import get_google_news
