...
class StockDataCache:
    ...
    def __init__(self, cache_dir: str = None):
        ...
        Args:
            cache_dir: 缓存目录路径，默认为 tradingscope/dataflows/data_cache
        """
        ...
        # 缓存配置 - 针对不同市场设置不同的TTL
        self.cache_config = {
            "us_stock_data": {"ttl_hours": 2, "max_files": 1000, "description": "美股历史数据"},  # 美股数据缓存2小时（考虑到API限制）
            "china_stock_data": {"ttl_hours": 1, "max_files": 1000, "description": "A股历史数据"},  # A股数据缓存1小时（实时性要求高）
            "us_news": {"ttl_hours": 6, "max_files": 500, "description": "美股新闻数据"},  # 美股新闻缓存6小时
            "china_news": {"ttl_hours": 4, "max_files": 500, "description": "A股新闻数据"},  # A股新闻缓存4小时
            "us_fundamentals": {"ttl_hours": 24, "max_files": 200, "description": "美股基本面数据"},  # 美股基本面数据缓存24小时
            "china_fundamentals": {"ttl_hours": 12, "max_files": 200, "description": "A股基本面数据"},  # A股基本面数据缓存12小时
        }
    ...
    def save_stock_data(self, symbol: str, data: Union[pd.DataFrame, str], start_date: str = None, end_date: str = None, data_source: str = "unknown") -> str:
        """
        ...
        market_type = self._determine_market_type(symbol)
        cache_key = self._generate_cache_key("stock_data", symbol, start_date=start_date, end_date=end_date, source=data_source, market=market_type)
    ...
    def find_cached_stock_data(
        self, symbol: str, start_date: str = None, end_date: str = None, data_source: str = None, max_age_hours: int = None
    ) -> Optional[str]:
        """
        ...
        # 生成查找键
        search_key = self._generate_cache_key("stock_data", symbol, start_date=start_date, end_date=end_date, source=data_source, market=market_type)
    ...
    def save_news_data(self, symbol: str, news_data: str, start_date: str = None, end_date: str = None, data_source: str = "unknown") -> str:
        """保存新闻数据到缓存"""
        # 检查内容长度是否需要跳过缓存
    ...
    def save_fundamentals_data(self, symbol: str, fundamentals_data: str, data_source: str = "unknown") -> str:
        ...
        market_type = self._determine_market_type(symbol)
        cache_key = self._generate_cache_key("fundamentals", symbol, source=data_source, market=market_type, date=datetime.now().strftime("%Y-%m-%d"))
    ...
    def find_cached_fundamentals_data(self, symbol: str, data_source: str = None, max_age_hours: int = None) -> Optional[str]:
        """
        查找匹配的基本面缓存数据
